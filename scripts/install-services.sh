...
echo aware2026 | sudo -S cp "$SCRIPT_DIR"/aware-bt-connect.timer /etc/systemd/system/
...
echo aware2026 | sudo -S systemctl enable aware-bt-connect.timer
echo aware2026 | sudo -S systemctl enable aware.service

echo "All services installed and enabled."
echo "Reboot to verify, or run: sudo systemctl start llama-server aware-bt-connect aware-bt-connect.timer aware"
