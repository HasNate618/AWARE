...
def _classify_sound(
    ...
    norm_energy = min(rms / (energy_threshold * 3), 1.0)
    ...
    for label, (fmin, fmax) in _FREQ_RANGES.items():
        ...
        if fmin <= dom_freq <= fmax:
            ...
            conf = norm_energy * 0.5 + dom_strength * 0.3
    ...
    # Also check spectral centroid for speech detection
    if 300 < centroid < 3400 and norm_energy > 0.2:
        has_speech = any(d[0] == "speech" for d in detections)
        if not has_speech:
            detections.append(("speech", round(norm_energy * 0.7, 3)))

    if not detections and rms > energy_threshold:
...
class YAMNetMic:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if self.device is None:
                ...
                for i, d in enumerate(devices):
                    if d["max_input_channels"] > 0 and "CAMERA" in d.get("name", "").upper():
                        self.device = i
                        logger.info("Found USB mic: %s (device %d)", d["name"], i)
                        break
                if self.device is None:
                    for i, d in enumerate(devices):
            ...
            self._stream.start()
            logger.info("Mic started (device=%s, rate=%d)", self.device, self.sample_rate)
    ...
    def _detect(self) -> PerceptionSnapshot:
        ...
        rms_val = _rms(audio)
        sounds_raw = _classify_sound(audio, self.sample_rate, rms_val, self.energy_threshold)
